...
fn run_broadcast_producer(
    ...
    run_producer_loop::<Item, _>(exit, report_prefix, total_items_produced, move || {
        ...
        for index in 0..batch.len() {
            ...
            // SAFETY: `index` is within the reserved batch.
            unsafe {
                batch.write_atomic(index, item, Ordering::Relaxed);
            }
        }
        Some(batch.len())
    });
...
fn run_broadcast_consumer(
    ...
    run_consumer_loop(exit, move || {
        // SAFETY: This benchmark uses the direct broadcast API only to advance
        // cursors and measure overwrite pressure. It does not dereference the
        // returned pointers; payload reads should use the broadcast atomic read
        // helpers or another atomic/external synchronization scheme.
        match unsafe { consumer.try_read_direct_batch(SYNC_CADENCE) } {
            Ok(Some(batch)) => {
                let _ = batch.len();
                if let Err(skipped) = batch.commit() {
                    let dropped = (skipped as u64).saturating_add(consumer.sync_to_latest() as u64);
                    consumer_reserve_failures.fetch_add(dropped, Ordering::Relaxed);
                }
            }
            Ok(None) => {}
            Err(skipped) => {
